      ...
      <motion.div ref={cardRef} className="relative mx-auto h-full md:h-auto w-full max-w-[700px] overflow-hidden md:rounded-2xl bg-[#1c1c1e] pointer-events-auto" layoutId={`card-container-${partial_project.slug}`}>
        <motion.div className="absolute h-[420px] w-full z-[1]" layoutId={`card-image-container-${project.id}`}>
          ...
          <div className="flex flex-col justify-center absolute bottom-[15px] left-[15px] space-y-2">
            ...
            <motion.div className={`uppercase font-bold text-xs text-white px-3 py-1 rounded ${projectStatusLabels[project.status].color}`} layoutId={`project-label-${project.id}`}>
          ...
          {project.link?.github && (
            <div className="flex flex-row absolute bottom-[15px] right-[15px] space-x-3 items-center">
              <button type="button" onClick={() => window.open(project.link?.github, '_blank')} className="flex items-center space-x-2 bg-blue-500 hover:bg-blue-400 text-white px-3 py-1 rounded">
        ...
        <motion.div className="absolute top-[30px] left-[30px] z-[1]" layoutId={`title-container-${project.id}`}>
          <div className="flex flex-row space-x-3 items-center">
            <h2 style={{ textShadow: '#aa0 1px 0 5px' }} className={`font-extrabold text-lg m-2 ${card_color}`}>
              {project.title}
            </h2>
